...
class Crossover2Way:
    def __init__(self, tFRD, wFRD, tZMA, wZMA):
        self.freq, self.tMag, self.tPhase = self.read_frd_file(tFRD)
        _, self.wMag, self.wPhase = self.read_frd_file(wFRD)
        self.tImp, self.tAngle = self.read_zma_file(tZMA)
        self.wImp, self.wAngle = self.read_zma_file(wZMA)

        self.th = 10 ** (self.tMag / 10) * np.exp(1j * np.pi / 180 * self.tPhase)
        self.wh = 10 ** (self.wMag / 10) * np.exp(1j * np.pi / 180 * self.wPhase)
        self.tz = self.tImp * np.exp(1j * np.pi / 180 * self.tAngle)
        self.wz = self.wImp * np.exp(1j * np.pi / 180 * self.wAngle)


    def highpass(self, r1, c1, l1):
        h = self.tz / (parallel(self.tz, l1.z) + r1.z + c1.z)
        zi = (r1.z + c1.z) + parallel(self.tz, l1.z)

        hMag = 20 * np.log10(np.absolute(h)) + self.tMag
        hPhase = np.angle(h, deg=True) + self.tPhase
        return h, zi, hMag, hPhase

    def lowpass(self, c1, l1, c2):
        ho = c2.z / (((l1.z * c1.z) / (l1.z + c1.z)) + c2.z)
        zi = (c1.z * l1.z) / (c1.z + l1.z) + (c2.z * self.wz) / (c2.z + self.wz)
        zo = ho * ((l1.z * c1.z) / (l1.z + c1.z))
        h = ho * self.wz / (self.wz + zo)

        hMag = 20 * np.log10(np.absolute(h)) + self.wMag
        hPhase = np.angle(h, deg=True) + self.wPhase
        return h, zi, hMag, hPhase
    ...
    def plot(self):
        ...
        ax1 = fig.add_subplot(211)
        plt.xscale('log')
        ax1.plot(self.freq, self.tMag, c='b')
        ax1.set_title('Tweeter Frequency Response')
        ax1.set_ylabel('dBSPL', color='b')
        ax1.set_xlim(20, 20000)
        ax1.set_ylim(50, 100)
        ...
        ax2 = ax1.twinx()
        plt.xscale('log')
        ax2.plot(self.freq, self.tPhase, c='r')
        ax2.set_ylabel('Phase (deg)', color='r')
        ax2.set_xlim(20, 20000)
        ax2.set_ylim(-190, 190)
        ...
        ax3 = fig.add_subplot(212)
        plt.xscale('log')
        ax3.plot(self.freq, self.wMag, c='b')
        ax3.set_title('Woofer Frequency Response')
        ax3.set_ylabel('dBSPL', color='b')
        ax3.set_xlim(20, 20000)
        ax3.set_ylim(50, 100)

        ax4 = ax3.twinx()
        plt.xscale('log')
        ax4.plot(self.freq, self.wPhase, c='r')
        ax4.set_ylabel('Phase (deg)', color='r')
        ax4.set_xlim(20, 20000)
        ax4.set_ylim(-190, 190)
...
def main():
    ...
    c = Crossover2Way(tweeterFRD, wooferFRD, tweeterZMA, wooferZMA)
    # c.plot()
    hu, zu, huMag, huPhase = c.highpass(r1, c3, l2)
    hl, zl, hlMag, hlPhase = c.lowpass(c1, l1, c2)

    freq = c.freq
    tOut = c.th * hu
    tOutMag = 10 * np.log10(np.abs(tOut))
    tOutPhase = np.angle(tOut)
    # huPhase = np.angle(hu, deg=True)
    #
    # hlMag = 20 * np.log10(np.absolute(hl))
    # hlPhase = np.angle(hl, deg=True)
    ...
    #
    # # THIS IS NOT CORRECT
    # h = 10**(daytonTweeter.amp / 10) * np.exp(2j * np.pi * daytonTweeter.phase) * hfh.h + 10**(daytonWoofer.amp / 10) * np.exp(2j * np.pi * daytonWoofer.phase) * tfh.h
    # # twoWayRawAmp = 10**(daytonTweeter.amp / 10) + 10**(daytonWoofer.amp / 10)
    # # twoWayPhasor = twoWayRawAmp * np.exp(2j * np.pi * twoWayPhaseU)
    # # o = twoWayPhasor * h
    # oMag = 10 * np.log10(np.absolute(h))
    # oPhase = np.mod(np.angle(h, deg=True) + 180, 360) - 180
    # oPhaseU = np.angle(h, deg=True)
    #
